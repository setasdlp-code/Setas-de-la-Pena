...
(function () {
  ...
  const assessReadiness = (input, model, history) => {
    ...
    const hasPreparationContract=Object.prototype.hasOwnProperty.call(input,'preparation');
    add('preparation','Especificación de preparación',preparation?.schemaVersion===1?'ready':hasPreparationContract?'blocked':'unknown',preparation?`${preparation.revision} · base seca · báscula ${preparation.weighing.resolutionG} g. Pesos planificados; confirma las mediciones.`:'Falta una especificación válida para estas cantidades.','batch','Revisar preparación');
    ...
    const quantityKnown = (!hasPreparationContract||!!preparation) && balanced && batchKnown && input.inventory?.available === true && stockMap && recipe.every(r => {
    ...
    if (quantityKnown) {
      const rows = recipe.map(r => {
        const requiredWetKg = preparation ? preparation.items.find(i=>i.ingredientId===r.id).inventoryKg : wetKg * (1 - moisture / 100) * Number(r.p) / 100 / (1 - Number(input.ingredientMoistureById[r.id]) / 100);
    ...
    if(preparation){
      const estimates=preparation.items.filter(i=>i.moisture.source!=='operator-measured').length;
      add('moisture-provenance','Humedad de ingredientes',estimates?'unknown':'ready',estimates?`${estimates} ingrediente(s) usan estimación de catálogo; no son mediciones del lote.`:'Valores de humedad ingresados por el operador para esta preparación.','batch','Revisar preparación');
      if(preparation.totals.waterExcessKg>0)add('water-excess','Exceso de agua','blocked',`Los ingredientes aportan ${preparation.totals.waterExcessKg.toFixed(3)} kg de agua por encima del objetivo calculado. No se puede retirar agua agregando menos.`,'batch','Revisar preparación');
    }
    ...
    return {inputRevision:input.inputRevision,preparationRevision:preparation?.revision??null,status:checks.some(c=>c.status==='blocked')?'blocked':checks.some(c=>c.status==='unknown')?'verify':'ready',checks,stock};
  };
