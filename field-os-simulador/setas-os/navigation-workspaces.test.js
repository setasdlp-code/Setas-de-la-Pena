...
test('Formular V2 separates Mesa and Generator without undefined replacement helpers', () => {
  ...
  assert.doesNotMatch(modeNav[0], /co-form-toggle/);
  assert.match(jsx, /id="co-form-toggle"[\s\S]*aria-pressed=\{coFormMode\}[\s\S]*aria-controls="co-form-panel"[\s\S]*aria-expanded=\{coFormMode\}/);
  assert.match(jsx, /<section id="co-form-panel" className="co-form-panel"[\s\S]*aria-labelledby="co-form-toggle"/);
