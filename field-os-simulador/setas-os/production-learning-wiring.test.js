...
test('bridge usa los contratos canónicos de ciclo, telemetría y evidencia', () => {
  ...
  assert.match(bridge, /SetasRoomCycle\.validateRoomCycle/);
  assert.match(bridge, /SetasTelemetry\.validateTelemetry/);
  assert.match(bridge, /buildCycleEvidence/);
});

test('materialización enlaza exclusivamente el lote correcto de Bitácora', () => {
  assert.match(bridge, /readJson\('sdp_bit_lotes'\)/);
...
test('CycleEvidence queda local y se sincroniza a Firestore sin duplicar identidad', () => {
...
test('Perito recibe historicalEvidence solo como contexto y resultado explicable', () => {
...
test('bridge no modifica el motor fuente ni el scoring', () => {
