...
Status: foundation + persistence vertical
...
`setas.telemetry.v1` normalizes `temperature_c`, `rh_pct`, `co2_ppm` and `substrate_temperature_c`. Every reading carries room, device, timestamp, quality and optional calibration provenance. Physically impossible readings are quarantined rather than silently averaged.

### `cycle-evidence.js`

`setas.cycle-evidence.v1` reuses `SetasBitacora.calcLoteStats()` for production outcomes and combines them with RoomCycle-scoped telemetry. It preserves recipe snapshot and physical ingredient-lot traceability when supplied.
...
`setas.experiment.v1` declares hypothesis, control, treatments, primary metric, replication, randomization and fixed/blocking factors before execution. A one-replicate trial remains explicitly `exploratory`; comparative evidence requires stronger design and complete traceability.

## Persistence vertical

The existing Firebase layer now persists the learning loop in three explicit collections:
...
The bridge keeps localStorage as the immediate operational cache and uses the existing Firebase layer for persistence. It:
...
6. persists evidence using `cycleId + batchId` as stable identity;
7. builds `setas.historical-evidence.v1` for the active species.

The bridge is loaded from `firebase/db.js` after `window.SetasDB` is published, keeping it independent of React navigation/component lifecycle.
...
The scenario result also exposes these fields for explanation/UI work. This version does **not** modify `scoring.js`, the source of `perito-scenarios.js`, `historyCalibration`, ranking weights or recipe selection. Production evidence is contextual only.

Any future calibration change must be separately validated against ground-truth production fixtures before it can influence ranking.

## Explicit non-goals

- no new navigation state or React UI in this vertical;
- no direct hardware transport ingestion yet;
- no autonomous actuator control;
- no scoring/ranking recalibration;
- no automatic promotion of observations into canonical agronomic claims;
