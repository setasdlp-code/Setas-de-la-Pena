...
(async()=>{
 ...
 try{
  ...
  await expect(page.getByRole('button',{name:/Planificar lote/})).toBeDisabled();
  ...
  await page.getByRole('button',{name:/Planificar lote/}).click();await expect(page.getByRole('dialog',{name:'Planificar lote',exact:true})).toBeVisible();
  ...
  await expect(page.getByRole('dialog',{name:'Planificar lote',exact:true})).toHaveCount(0);
  ...
  const accepted=await spec();await page.getByRole('button',{name:/Planificar lote/}).click();
  const dialog=page.getByRole('dialog',{name:'Planificar lote',exact:true});await expect(dialog).toContainText(accepted.revision);
  ...
  // sigue entero y los kilos del plan aparecen comprometidos.
  const inventory=await page.evaluate(()=>JSON.parse(localStorage.getItem('sdp_lotes')));
  assert.equal(inventory.find(l=>l.ingredienteId==='paja_trigo').cantidadKgDisponible,100);
  const held=await page.evaluate(()=>JSON.parse(localStorage.getItem('sdp_inv_reservas')||'[]').filter(r=>r.status==='held'));
  assert.ok(held.length>0,'planificar no dejó reservas');
  const pajaHeld=held.filter(r=>r.ingredienteId==='paja_trigo').reduce((s,r)=>s+r.kg,0);
  assert.ok(Math.abs(pajaHeld-accepted.items[0].inventoryKg)<0.01,
    `reservado ${pajaHeld} kg de paja frente a ${accepted.items[0].inventoryKg} kg del plan`);
