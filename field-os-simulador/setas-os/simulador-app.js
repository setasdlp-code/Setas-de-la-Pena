...
Click para ver análisis completo`
      ...
      { id: "gen-panel", l: "Generador", icon: IconBolt },
...
Click para editar`
    ...
    return ROLE_GROUPS.map((grp) => {
      ...
      // reducir scroll. Una vez el usuario toca el acordeón, su elección explícita
      // (true/false en collapsedRoles) manda sobre este default.
      const hasSignal = compatCount > 0 || grpIngs.some((g) => recipe.some((r) => r.id === g.id));
      const isCollapsed = collapsedRoles[grp.key] ?? !hasSignal;
    ...
    }, "aria-label": `Quitar ${g?.name || "ingrediente"} de la receta`, style: { flexShrink: 0, fontSize: "var(--text-base)", padding: "4px 8px" } }, "✕")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 4 } }, /* @__PURE__ */ React.createElement("input", { type: "range", min: "0", max: "100", step: ".5", value: r.p, onChange: (e) => !isLocked && updP(r.id, parseFloat(e.target.value) || 0), disabled: isLocked, "aria-label": `Porcentaje de ${g.name}`, "aria-valuetext": `${r.p}%`, "aria-disabled": isLocked, style: { opacity: isLocked ? 0.5 : 1, width: "100%" } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, justifyContent: "space-between" } }, /* @__PURE__ */ React.createElement("input", { type: "number", inputMode: "decimal", min: "0", max: "100", step: ".5", value: r.p, onChange: (e) => !isLocked && updP(r.id, parseFloat(e.target.value) || 0), readOnly: isLocked, className: "rec-pct-input", style: { width: "70px", padding: "6px 8px", border: "1px solid var(--paper-300)", background: isLocked ? "var(--paper-200)" : "var(--paper-100)", fontFamily: "var(--font-mono)", fontSize: "var(--text-sm)", textAlign: "center", color: "var(--ink-900)", outline: "none", borderRadius: "var(--r-xs)" } }), /* @__PURE__ */ React.createElement("span", { className: "pct", style: { fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--ink-600)" } }, "%"))));
