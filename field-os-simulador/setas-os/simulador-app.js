...
// source-hash: 71d93fa671cc5b68854492bf73687c7c16e1d42535856791a228b0928d7f14fa
...
Click para ver análisis completo`
    ...
    return /* @__PURE__ */ React.createElement("div", { key: r.id, className: `rec-row${isLocked ? " rec-locked" : ""}`, style: { display: "flex", flexDirection: "column", gap: 8, padding: "12px 14px", borderBottom: "1px solid var(--paper-300)" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "flex-start", gap: 6, justifyContent: "space-between" } }, /* @__PURE__ */ React.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 5, marginBottom: 3 } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: "var(--text-base)", fontWeight: 500 } }, g.name), /* @__PURE__ */ React.createElement("button", { type: "button", className: `lock-btn mix-lock-btn${isLocked ? " on" : ""}`, onClick: () => toggleLock(r.id), "aria-label": isLocked ? `Desbloquear porcentaje de ${g?.name || ""}` : `Fijar porcentaje de ${g?.name || ""}`, title: isLocked ? "Desbloquear (incluir en auto-ajuste)" : "Fijar este % (excluir del auto-ajuste)" }, /* @__PURE__ */ React.createElement("span", { "aria-hidden": "true" }, isLocked ? "●" : "○"), " ", isLocked ? "Fijado" : "Libre")), /* @__PURE__ */ React.createElement("div", { className: "imeta", style: { fontSize: "var(--text-xs)" } }, "C:N ", g.cn || "—", " · N ", g.n || "—", "%"), rowFlag && /* @__PURE__ */ React.createElement("div", { style: { marginTop: 4, fontSize: "var(--text-xs)", fontWeight: 700, color: rowFlag.priority === "critical" ? "var(--coral-500)" : "#7A5A10", display: "flex", alignItems: "center", gap: 4 } }, /* @__PURE__ */ React.createElement("span", null, rowFlag.priority === "critical" ? "⚠" : "!"), /* @__PURE__ */ React.createElement("span", null, rowFlag.label))), /* @__PURE__ */ React.createElement("button", { type: "button", className: "rem mix-remove-btn", onClick: () => {
      remI(r.id);
      setLockedIds((l) => l.filter((x) => x !== r.id));
    }, "aria-label": `Quitar ${g?.name || "ingrediente"} de la receta` }, "✕")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 4 } }, /* @__PURE__ */ React.createElement("input", { type: "range", min: "0", max: "100", step: ".5", value: r.p, onChange: (e) => !isLocked && updP(r.id, parseFloat(e.target.value) || 0), disabled: isLocked, "aria-label": `Porcentaje de ${g.name}`, "aria-valuetext": `${r.p}%`, "aria-disabled": isLocked, style: { opacity: isLocked ? 0.5 : 1, width: "100%" } }), /* @__PURE__ */ React.createElement("div", { className: "mix-steppers", role: "group", "aria-label": `Ajustar porcentaje de ${g.name}` }, [-5, -1].map((delta) => /* @__PURE__ */ React.createElement("button", { key: delta, type: "button", className: "mix-step-btn", disabled: isLocked || Number(r.p) <= 0, onClick: () => updP(r.id, Math.max(0, Math.min(100, (parseFloat(r.p) || 0) + delta))) }, delta, "%")), /* @__PURE__ */ React.createElement("label", { className: "mix-number-wrap" }, /* @__PURE__ */ React.createElement("span", { className: "sr-only" }, "Porcentaje de ", g.name), /* @__PURE__ */ React.createElement("input", { type: "number", min: "0", max: "100", step: ".5", inputMode: "decimal", required: true, value: r.p, onChange: (e) => !isLocked && updP(r.id, parseFloat(e.target.value) || 0), readOnly: isLocked, "aria-label": `Porcentaje de ${g?.name || "ingrediente"} (numérico)`, className: "rec-pct-input mix-num-input" }), /* @__PURE__ */ React.createElement("span", { "aria-hidden": "true" }, "%")), [1, 5].map((delta) => /* @__PURE__ */ React.createElement("button", { key: delta, type: "button", className: "mix-step-btn", disabled: isLocked || Number(r.p) >= 100, onClick: () => updP(r.id, Math.max(0, Math.min(100, (parseFloat(r.p) || 0) + delta))) }, "+", delta, "%")))));
