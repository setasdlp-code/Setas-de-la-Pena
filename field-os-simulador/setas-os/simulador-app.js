...
// source-hash: c0604672d19404ae26fbd53e9363c263cb967caeae095c529c15e36139e7c921
...
Click para ver análisis completo`
    ...
    }, "aria-label": `Quitar ${g?.name || "ingrediente"} de la receta`, style: { flexShrink: 0, fontSize: "var(--text-base)", padding: "4px 8px" } }, "✕")), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 4 } }, /* @__PURE__ */ React.createElement("input", { type: "range", min: "0", max: "100", step: ".5", value: r.p, onChange: (e) => !isLocked && updP(r.id, parseFloat(e.target.value) || 0), disabled: isLocked, "aria-label": `Porcentaje de ${g.name}`, "aria-valuetext": `${r.p}%`, "aria-disabled": isLocked, style: { opacity: isLocked ? 0.5 : 1, width: "100%" } }), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, justifyContent: "space-between" } }, /* @__PURE__ */ React.createElement("input", { type: "number", min: "0", max: "100", step: ".5", inputMode: "decimal", required: true, value: r.p, onChange: (e) => !isLocked && updP(r.id, parseFloat(e.target.value) || 0), readOnly: isLocked, "aria-label": `Porcentaje de ${g?.name || "ingrediente"} (numérico)`, className: "rec-pct-input", style: { width: "70px", padding: "6px 8px", border: "1px solid var(--paper-300)", background: isLocked ? "var(--paper-200)" : "var(--paper-100)", fontFamily: "var(--font-mono)", fontSize: "var(--text-sm)", textAlign: "center", color: "var(--ink-900)", borderRadius: "var(--r-xs)" } }), /* @__PURE__ */ React.createElement("span", { className: "pct", style: { fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--ink-600)" } }, "%"))));
