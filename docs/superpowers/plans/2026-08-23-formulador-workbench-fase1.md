...
> **UPDATE 2026-08-23 (re-scope tras drift de main):** Task 1 (persistencia de borrador) quedó **superseded** por PR #97 `codex/formulator-draft-recovery` (merge `d1ee8d1`), mergeado por otro agente en paralelo mientras se ejecutaba este plan. Su implementación (`readFormDraft()` + lazy `useState` initializers, misma técnica a la que este plan llegó tras su propia ronda de fix) es más completa que la de este plan: deduplica ingredientes, clampa porcentajes 0-100, y restaura `saveName` además de receta/especie/bloqueados. **Task 1 no se re-ejecuta.** Tasks 2-4 continúan, re-verificadas contra el `main` actual (ver ledger `.superpowers/sdd/2026-08-23-formulador-workbench-fase1/progress.md` para el detalle del drift encontrado: PRs #89-#97, incluyendo un split quirúrgico del mezclador/generador que renombró `balanced`/`balMsg` a `readyForProduction`/`productionBlockMsg` en el bloque `.sbar` que Task 3 toca — la variable base `balanced` sigue existiendo, solo cambió cuál se usa en ese banner específico).
